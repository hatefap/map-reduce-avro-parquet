...
public class SortParquet extends Configured implements Tool {
    ...
    public static class ParquetReducer extends Reducer<LongDescendComparable, NullWritable, Void, SpecificRecord>{

        private final CustomAvroSchema.Builder builder = CustomAvroSchema.newBuilder();
        @Override
        public void reduce(LongDescendComparable key, Iterable<NullWritable> values, Context context) throws IOException, InterruptedException {
            CustomAvroSchema record = builder.setRandomGeneratedNumber(key.get()).build();
            context.write(null, record);
        }
    }

    public static class ParquetPartitioner extends Partitioner <LongDescendComparable, NullWritable> {
        BigInteger M = new BigInteger(String.valueOf(Long.MAX_VALUE));
        BigInteger m = new BigInteger(String.valueOf(Long.MIN_VALUE));
        BigInteger diff = M.subtract(m);
        @Override
    ...
    public int run(String[] args) throws IOException, ClassNotFoundException, InterruptedException {
        ...
        AvroParquetOutputFormat.setBlockSize(job, BLOCK_SIZE);
